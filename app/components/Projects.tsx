...
const Projects = () => {
  ...
  return (
    <section id="projects" className="section-padding bg-ink corps-texture">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <SectionHeading
          eyebrow="Mission Files"
          title={<>My <span className="text-crimson-bright">Projects</span></>}
          subtitle="Here are some of my recent projects that showcase my skills and passion for development"
        />

        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <Reveal key={index} delay={(index % 2) * 100} className="dossier-card rounded-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-2 bg-ink/60 border-b border-gold/15">
                <span className="font-label text-[11px] tracking-[0.18em] uppercase text-gold/80">
                  File No. {String(index + 1).padStart(2, '0')}
                </span>
                <span
                  className={`stamp text-[10px] ${project.live ? 'text-corps-green border-corps-green/70' : 'text-gold/70 border-gold/50'}`}
                >
                  {project.live ? 'Deployed' : 'Archived'}
                </span>
              </div>
              <div className="relative group">
                ...
                {project.hasVideo && (
                  <button onClick={() => setSelectedProject(project)}
                    className="absolute inset-0 bg-ink/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    title="Watch demo"
                  >
                    <div className="bg-gold rounded-full p-4">
                      <Play className="text-ink" size={24} />
                    </div>
                  </button>
                )}
              </div>
              ...
              <div className="p-6">
                <h3 className="font-display text-xl font-bold text-parchment mb-3">{project.title}</h3>
                <p className="text-parchment-dim mb-4 leading-relaxed">{project.description}</p>

                <div className="flex flex-wrap gap-2 mb-4">
                  {project.technologies.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className="bg-gold/10 border border-gold/30 text-gold px-3 py-1 rounded-full text-sm font-medium"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                <div className="flex gap-4">
                  <a
                    href={project.github}
                    className="flex items-center gap-2 text-parchment-dim hover:text-gold transition-colors"
                  >
                    <Github size={18} />
                    Code
                  </a>
        ...
        {selectedProject && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-ink-soft border border-gold/30 rounded-sm p-6 max-w-4xl w-full max-h-[90vh] overflow-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-display text-2xl font-bold text-parchment">{selectedProject.title}</h3>
                <button
                  onClick={() => setSelectedProject(null)}
                  className="text-parchment-dim hover:text-gold text-2xl"
                >
                  ×
                </button>
              </div>
